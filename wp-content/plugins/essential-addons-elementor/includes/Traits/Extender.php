...
trait Extender {
	...
	public function add_team_member_circle_presets( $obj ) {
		$obj->add_responsive_control( 'eael_team_members_image_height', [
		...
		$obj->add_responsive_control( 'eael_team_members_circle_image_width', [
		...
		$obj->add_responsive_control( 'eael_team_members_circle_image_height', [
			'label'      => esc_html__( 'Image Height', 'essential-addons-elementor' ),
			'type'       => Controls_Manager::SLIDER,
			'default'    => [
				'size' => 150,
	...
	public function lr_init_content_pass_strength_controls( Login_Register $lr ) {
		...
		$lr->add_control( 'show_pass_strength', [
			'label' => __( 'Show Password Strength', 'essential-addons-elementor' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
		$lr->add_control( 'ps_text_type', [
			'label'     => esc_html__( 'Password Strength Text', 'essential-addons-elementor' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => 'default',
			'options'   => [
		...
		foreach ( $pass_type as $p_type ) {
			$lr->add_control( "ps_text_{$p_type}", [
				/* translators: %s: Strength of the Password eg. Bad, Good etc. */
				'label'       => sprintf( __( '%s Password', 'essential-addons-elementor' ), ucfirst( $p_type ) ),
				'type'        => Controls_Manager::TEXT,
				/* translators: %s: Strength of the Password eg. Bad, Good etc. */
				'default'     => sprintf( __( '%s Password', 'essential-addons-elementor' ), ucfirst( $p_type ) ),
	...
	public function lr_init_style_pass_strength_controls( Login_Register $lr ) {
		$lr->start_controls_section( 'section_style_pass_strength', [
			'label'     => __( 'Password Strength Meter', 'essential-addons-for-elementor' ),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'show_pass_strength' => 'yes',
			],
		...
		$lr->add_responsive_control( "eael_ps_wrap_width", [
			...
			'selectors'  => [
				$notice_container => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$lr->add_responsive_control( "eael_ps_wrap_height", [
			...
			'selectors'  => [
				$notice_container => 'height: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$lr->add_responsive_control( "eael_ps_wrap_padding", [
			...
			'selectors'  => [
				$notice_container => $lr->apply_dim( 'padding' ),
			],
			'separator'  => 'after',
		] );
		$lr->add_group_control( Group_Control_Typography::get_type(), [
			'name'     => 'eael_ps_wrap_typo',
		...
		$lr->add_control( "eael_ps_wrap_text_color", [
			'label'     => __( 'Text Color', 'essential-addons-for-elementor' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				$notice_container => 'color: {{VALUE}} !important;',
			],
			'separator' => 'before',
		] );
		$lr->add_group_control( Group_Control_Background::get_type(), [
	...
	public function lr_show_password_strength_meter( Login_Register $lr ) {
		$ss = $lr->get_settings_for_display( 'show_pass_strength' );
		if ( 'yes' !== $ss ) {
			return;// vail early if SPS is not set to 'yes'.
		}
		$pass_types = [
			'bad',
			'good',
		...
		$data       = [
			'show_pass_strength' => esc_attr( $ss ),
		...
		$encoded = esc_attr( json_encode( $data ) );
		echo "<p class='eael-pass-notice' style='display: none' data-strength-options='{$encoded}'></p>";
